...
function filterGames(keyword) {
  ...
  cards.forEach((card) => {
    ...
    const isMatch = title.includes(keyword) || genre.includes(keyword) || specs.includes(keyword);
...
function handleSearch() {
  ...
  const katalogPath = isHomePage ? 'html/katalog.html' : 'katalog.html';
  window.location.href = `${katalogPath}?q=${encodeURIComponent(keyword)}`;
}
